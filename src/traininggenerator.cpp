...
TrainingGenerator::TrainingGenerator(KeyboardLayout layout, KeyboardStandard standard, QObject* parent)
...
TrainingGenerator::TrainingGenerator(QString& u, QString& m, QString& l, KeyboardStandard standard, QObject* parent)
        : QObject(parent), standard(standard)
{
        rows.append(u);
        rows.append(m);
        rows.append(l);
        ...
        if (l.size() == 10) {
                standard = KeyboardStandard::ANSI;
        } else if (l.size() == 11) {
                standard = KeyboardStandard::ISO;
        }
}
...
QString TrainingGenerator::generateLesson(QString& characters, int wordLength, int maxLength)
{
        QString lesson;
