...
void DB::initDB() {
    ...
    try {
        ...
        sqlite3pp::transaction xct(db);
        {
            ...
            db.execute("CREATE TABLE text(id INTEGER PRIMARY KEY AUTOINCREMENT,"
            ...
            db.execute("CREATE TABLE statistic (w DATETIME, data text,"
                       "type integer,time real,count integer, mistakes integer,"
                        "viscosity real)");
            db.execute("CREATE TABLE mistake (w DATETIME, target text,"
                       "mistake text, count integer)");
            db.execute("CREATE VIEW text_source as SELECT id, s.name, text,"
...
void DB::disableSource(const QList<int>& sources) {
    ...
    sqlite3pp::transaction xct(db);
    {
        sqlite3pp::command cmd(
            db, "UPDATE text SET disabled = 1 where source = ?");
...
void DB::addResult(const QString& time, const Text* text,
...
void DB::addStatistics(const QString& time,
...
Text* DB::getNextText() {
    ...
    QStringList row2 = DB::getOneRow("select rowid from text where id = ?",
                                     row[0]);
...
Text* DB::getTextWithQuery(const QString& query) {
    try {
        ...
        if (data[5].isEmpty())
            data[5] = "0";
        return new Text(data[0].toUtf8(), data[1].toInt(), data[2], data[3],
